...
class _MainPageState extends State<MainPage> {
  ...
  // String cek_hurufPertamaKalimat(String text){
  //   final kalimat = text.split('.');
  //   for (int i = 0; i < kalimat.length; i++){
  //     if(kalimat.isNotEmpty){
  //       kalimat[i] = kalimat[i][0].toUpperCase() + kalimat[i].substring(1);
  //     }
  //   }
  //   return kalimat.join('. ');
  // }
        ...
        Row(
          ...
          children: [
            ElevatedButton(
              ...
              onPressed: name.isNotEmpty && number.isNotEmpty?
                  (){
                setState(() {
                  contacts.add(ContactModel(name: name, number: int.parse(number)));
                  contacts.forEach((element) {
                    print('Nama: ${element.name}, Nomor: ${element.number}');
                  });
                  nameController.text = '';
